...
public class RebuiltLEDCommand extends Command {
  ...
  public void execute() {
    ...
    throttleTracker = 0;
    // led.reset();
    if (!RobotState.isDSAttached()) { // when not conected
      led.setControl(Animations.kDisconnectedVertical);
      led.setControl(Animations.kDisconnectedHorizontal);
      wasDisabled = true;
    } else if (RobotState.isDisabled()) { // when connected and not enabled
      led.setControl(Animations.kDisconnectedVertical);
      ...
      if (RobotState.isAutonomousEnabled()) { // autonomous fire
